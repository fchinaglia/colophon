...
> Register: 81 events, root ae68ae8dc078c46c6ac85b349ae08d2d104fea77767dce18a7b38a5388312793. Ed25519 signature, RFC 3161 timestamp and Bitcoin anchoring alongside the register. Register and verification instructions: github.com/fchinaglia/colophon/tree/main/cases/002.
...
**The root goes in whole.** All sixty-four characters. An abbreviation can be recognised but not compared, and comparing is the entire job: the reader recomputes the chain and holds their value against yours. At 7pt the full root takes about ninety millimetres, which the column has. `--short-root` exists for a social card or a slide, where the line has to survive at any cost, and nowhere else.

**And it carries an address.** A line ending in *verification instructions in VERIFY.md* helps a reader standing in the case folder — the one reader who did not need help. Everyone else is on a post, a PDF, a printed page, and for them a filename is not a place. Put the published location in `case.json` as `register_url` and `build_note.py` prints it; the script warns on stderr when it finds neither that nor `--url`, because a line with no address is the failure mode this line exists to remove, wearing the costume of the fix.
