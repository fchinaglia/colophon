...
and the timestamp. The only route, and the one that needs nobody alive in ten years.

**There is no address, and none is offered** — not a hosted copy, not a deposit service,
not a link to somewhere the case supposedly lives. `case.json` has no field for one and the
scripts read none: an address is a promise about a server somebody keeps renewing, and a
...
in. And drop the copy you are about to send on `verify.html` yourself: it opens the PDF
and says what is actually inside it.
