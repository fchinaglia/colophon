...
A case is not finished when it is sealed. It is finished when the address it declares
answers.

That address is in `case.json` from the opening, so everything downstream is confident
about it: the page links the register, the technical line prints the address, the
manifest freezes both. Nothing anywhere says *and this has not been published yet*. A
reader who follows the line gets a 404 under a signed declaration, and from where they
stand that is not distinguishable from a case withdrawn, forged, or never made.

The client is `cli/colophon.py`, in the repository and not in the case folder, because it
holds the author's key and a case must stay verifiable without one:

```bash
python3 cli/colophon.py address <case_uid>     # the address, before anything is at it
python3 cli/colophon.py deposit cases/NNN-<slug> --to <instance> --invite <code>
```

It sends what the manifest covers, plus the seal — which no manifest can cover, because
it comes after — and the files written for a reader. The drafts in `versions/` are
refused: the register commits to their digests, so they stay attested without being
revealed. The address is derived from `case_uid` and the author's secret, never from the
root, which is why `case.json` can carry it before the case exists.

Publication records no event. Like the technical line, it happens after the last one: a
new event would change the root the line prints and the manifest already covers.

**Then check that the address answers, before you tell the author the case is done.**
...
the address serves the bytes the signature attests. A different one, or a `curl` that
fails, means the case is not published, whatever the deposit printed on its way out.

**An address that has not been published is not done.** Do not report the closing
sequence as successful while the declared address 404s. That the numbers are right is not
what the note claims: it tells a reader where to go and check them, and a note whose
address is dead is the failure the note exists to remove, wearing the costume of the fix.

**Say what is not yet true.** A publication that is deferred — the instance is not ready,
the domain does not exist yet, the author wants to hold the piece — is recorded, and
*when* it is recorded is the whole difficulty. Before the manifest it is one event like
any other. After the seal it is a reopening: a new event saying why, a new manifest, a
second signature, the old seal kept. So find out early — try the instance before you
compute the manifest, not after it. What is not allowed is the third option: leaving a
declared address that reads as live and saying nothing. That is a silent gap, and it is
the one a reader walks into.
