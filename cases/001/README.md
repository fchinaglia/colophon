...
| Register | 80 events, root `e9b049198f2849837d8c12057e516ade674316e12efd59b876a1525b5276272b` |
...
verification page, the icon, and every script a reader runs to check any of it — so
hashing those and finding them inside the signed register closes the chain. All sixteen
match. Outside it by design: `README.md` and `index.html`, prose about the case, and
`post.html`, `post.pdf` and `render_post.py`, which are the publication. A rendering is
derivable from what is covered, and freezing one means reopening a sealed case every
time it has to be made again.
...
Two more things the first reopening produced, both recorded. The coverage check stopped the
run on the reopening's own script-update event, because it counted a `meta` event as a
change to the text: a defect in the tool, fixed in the skill and noted in the register. And
an event written during the work named a change `R18b`, which does not exist; append-only
means it was corrected by a later event and not erased.

**Then the case was reopened a second time, the same day.** `post.html` and `post.pdf`
carried a technical line typed by hand — *71 eventi, radice 6fa3e24f* — and that hash is
the one at seq 70: the manifest event came after it, so the published document was
inaccurate by one event on the day it was made. It also pointed at an `icon.png` that does
not exist in this folder, so the icon had never rendered in it, and it gave no address for
the verification page. `render_post.py` now builds both from the measured text, with the
technical line generated by `build_note.py` after the seal — which is the rule the first
rendering broke.

The second reopening also corrected a design error of mine: the renderings were listed in
the manifest, carried forward without question from the manifest before it. Freezing a
rendering means reopening a sealed case whenever it has to change, which is what had just
happened, twice. Manifest v5 covers what the measurement depends on and what a verifier
runs; the publication sits outside it.

## The two seals

The seal of 22 August is kept as `events.jsonl.v1.{sig,tsr,ots,sha256}`. It verifies
against the first 72 events — the register as it stood then — and it is not obsolete: it
